...
void* rand_uint8_to_string(const uint8_t* rand_uint8,char rand_str[32]) {
    for (int i = 0; i < 16; i++) {
        sprintf(rand_str + i * 2, "%02x", rand_uint8[i]);
    }
}

/*
void send_rand_to_server(uint8_t *rand_uint8){
	int socket_desc;
	...
	rand = uint8_to_string(rand_uint8);
    printf("send rand to server: %s.......", rand);
    
    send(socket_desc, rand, strlen(rand), 0);
    printf("Complete!\n");
    close(socket_desc);
    return;
}
...
void receive_sres_from_server(uint8_t sres[4],uint8_t *rand_uint8){
	char byteStr[3] = {0};
	char rand_str[32];
	int sock;
	...
	rand_uint8_to_string(rand_uint8,rand_str);
    printf("Send rand to server: %s\n", rand_str);
	send(sock, rand_str, strlen(rand_str), 0);
    
    read(sock, buffer, 8);
    printf("Received sres from server: %s\n", buffer);
	close(sock);
	
	
	byteStr[0] = buffer[0];
    byteStr[1] = buffer[1];
    sres[0] = (uint8_t)strtol(byteStr, NULL, 16);

    byteStr[0] = buffer[2];
    byteStr[1] = buffer[3];
    sres[1] = (uint8_t)strtol(byteStr, NULL, 16);

    byteStr[0] = buffer[4];
    byteStr[1] = buffer[5];
    sres[2] = (uint8_t)strtol(byteStr, NULL, 16);

    byteStr[0] = buffer[6];
    byteStr[1] = buffer[7];
    sres[3] = (uint8_t)strtol(byteStr, NULL, 16);
	

	return;
}

void print_sres(uint8_t sres[4]){
	printf("=============RAND Information==========\n");
	printf("SRES[%d] = 0x%x\n", 0, sres[0]);
	printf("SRES[%d] = 0x%x\n", 1, sres[1]);
	printf("SRES[%d] = 0x%x\n", 2, sres[2]);
	printf("SRES[%d] = 0x%x\n", 3, sres[3]);
	...
	uint8_t *rand, uint8_t no_sim)
{
	...
	if (!GSM_SIM_IS_READER(subscr->sim_type)
	 || !subscr->sim_valid || no_sim) {
		...
		//printf("received rand: %s\n", uint8_to_string(rand));
		

		printf("=============Mobile Information========\n");
		printf("IMSI: %s\n", set->test_imsi);
		printf("IMEI: %s\n", set->imei);
		printf("=======================================\n");
		
		
		receive_sres_from_server(sres_uint8,rand);
		
		nmme->sres[0] = sres_uint8[0];
		nmme->sres[1] = sres_uint8[1];
		nmme->sres[2] = sres_uint8[2];
		nmme->sres[3] = sres_uint8[3];
		
		print_sres(nmme->sres);
